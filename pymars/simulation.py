...
class Simulation(object):
    ...
    def setup_case(self):
        ...
        """
        if type(self.model) == ct.Solution:
            self.gas = self.model
        else:
        ...
        self.ign_rise = 400.0
        if self.properties.ign_rise:
            self.ign_rise = self.properties.ign_rise
        ...
        # self.sim.preconditioner = precon  # Add it to the network
        ...
        with tables.open_file(self.save_file, mode='w',
            ...
            if self.time_end:
                ...
                while self.sim.time < self.time_end:
                    ...
                    if self.reac.T >= self.properties.temperature + self.ign_rise and not ignition_flag:
    ...
    def calculate_ignition(self):
        ...
        if self.time_end:
            ...
            while self.sim.time < self.time_end:
                self.sim.step()
                if self.reac.T >= self.properties.temperature + self.properties.ign_rise:
                    self.ignition_delay = self.sim.time
                    break
    ...
    def process_results(self, skip_data=False):
        ...
        for time, temp, pres, mass in zip(
            ...
            if temp >= temperature_initial + self.properties.ign_rise and not ignition_flag:
